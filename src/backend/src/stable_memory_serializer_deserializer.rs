...
pub fn serialize_to_stable_memory<S: Serialize>(state: S) -> Result<(), impl Error> {
    let writer = BufferedStableWriter::new(BUFFER_SIZE_BYTES);
    serialize(state, writer)
}

pub fn deserialize_from_stable_memory<S: DeserializeOwned>() -> Result<S, impl Error> {
    let stable_size = ic_cdk::api::stable::stable_size() as usize * WASM_PAGE_SIZE_IN_BYTES;
    let buffer_size = min(BUFFER_SIZE_BYTES, stable_size);
